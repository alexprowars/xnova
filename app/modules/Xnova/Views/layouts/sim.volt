...
<form method="post" action="<?=$this->url->get('xnsim/report/'.($this->config->view->get('socialIframeView', 0) ? '?ingame' : '')) ?>" name="form" id="result" autocomplete="off" <?=($this->user->getUserOption('ajax_navigation') ? 'class="noajax"' : '') ?> target="_blank">
