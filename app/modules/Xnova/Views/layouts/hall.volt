	...
	<tr>
		...
		<td class="c" width="137">
			<form method="POST" action="{{ url('hall/') }}" id="hall"><select name="visible" onChange="$('#hall').submit()" title="">
				<option value="1" <?=((!isset($_POST['visible']) or $_POST['visible'] <= 1) ? 'selected' : '') ?>>Бои
				<option value="2" <?=((isset($_POST['visible']) and $_POST['visible'] == 2) ? 'selected' : '') ?>>САБ
			</select></form>
		</td>
	</tr>
</table>
<div class="separator"></div>
	<table class="table">
		<tr>
			...
			<td class="c"><font color=#CDB5CD><?=((!isset($_POST['visible']) or $_POST['visible'] <= 1) ? 'Самые разрушительные бои' : 'Самые разрушительные групповые бои') ?></font></td>
		...
		for ($parse['hall'] AS $log): $i++; ?>
			<tr>
				...
				<th nowrap>{% if parse['time'] == $log['time'] %}<font color="green">{% endif %}{{ game.datezone("d.m.y H:i", $log['time']) }}{% if parse['time'] == $log['time'] %}</font>{% endif %}</th>
			</tr>
			{% endfor %}
		{% else %}
		<tr>
