...
class User
{
	...
	public static function deleteById ($userId)
	{
		...
		$planets = $db->query('SELECT id FROM '.DB_PREFIX.'planets WHERE id_owner = ?', [$userId]);

		while ($planet = $planets->fetch())
		{
			$db->delete(DB_PREFIX.'planets_buildings', 'planet_id = ?', [$planet['id']]);
			$db->delete(DB_PREFIX.'planets_units', 'planet_id = ?', [$planet['id']]);
		}
		...
		$db->delete(DB_PREFIX.'users_quests', 'user_id = ?', [$userId]);
