...
class PlayersAdminController extends Application
{
	public function indexAction ()
	{
		if ($this->user->authlevel >= 1)
		{
			...
			if ($this->request->get('result', 'string', '') != '')
			{
				...
				switch ($result)
				{
					case 'usr_data':
						...
						if ($this->user->authlevel >= 1)
						{
							...
							$this->view->pick('admin/adminpanel_ans1');
					...
					case 'ip_search':
						...
						$this->view->pick('admin/adminpanel_ans2');
						$this->view->setVar('parse', $parse);
						break;
					default:
						break;
			...
			elseif ($this->request->get('mode', 'string', '') != '')
			{
				switch ($this->request->get('mode', 'string', ''))
				{
					case 'usr_data':

						if ($this->user->authlevel >= 1)
							$this->view->pick('admin/adminpanel_f4');
						else
					...
					case 'usr_level':
						...
						if ($this->user->authlevel >= 3)
							$this->view->pick('admin/adminpanel_f3');
						else
							$this->message(_getText('sys_noalloaw'), _getText('sys_noaccess'));
					...
					case 'ip_search':
						...
						$this->view->pick('admin/adminpanel_f2');

						break;
				}
			}
			else
				$this->view->pick('admin/adminpanel');
