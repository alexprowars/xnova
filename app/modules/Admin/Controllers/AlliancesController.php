...
class AlliancesController extends Controller
{
	...
	public function indexAction ()
	{
		...
		if (isset($_GET['desc']))
		{
			...
			$info = $this->db->query("SELECT `description` FROM game_alliance WHERE id='" . $ally_id . "'");
		...
		if (isset($_GET['allyname']))
		{
			...
			$u = $this->db->query("SELECT `image`, `web`, `name`, `tag` FROM game_alliance WHERE `id` = '" . $ally_id . "'")->fetch();
		...
		if (isset($_GET['mitglieder']))
		{
			...
			$users = $this->db->query("SELECT `id`, `username` FROM game_users WHERE ally_id='" . $ally_id . "'");
		...
		if (isset($_GET['mail']))
		{
			...
			if (isset($_POST['text']))
			{
				...
				while ($u = $sq->fetch())
				{
					$this->db->query("INSERT INTO game_messages SET
											`owner`='{$u['id']}',
		...
		if (isset($_GET['leader']))
		{
			...
			$query = $this->db->query("SELECT `owner` FROM game_alliance");
			$u = $query->fetch();
			$users = $this->db->query("SELECT `username` FROM game_users WHERE id='" . $u['owner'] . "'");
			$a = $users->fetch();
			$leader = $a['username'];
			...
			if (isset($_POST['leader']))
			{
				...
				{
					$this->db->query("UPDATE game_alliance SET `owner` = '" . intval($_POST['leader']) . "' WHERE `id` = '" . intval($_GET['leader']) . "'");
				}
