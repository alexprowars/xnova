...
use App\Engine\Battle\Engine\Utils\Gauss;
use App\Engine\Battle\Engine\Utils\GeometricDistribution;
use App\Engine\Battle\Engine\Utils\Math;
use App\Engine\Battle\Engine\Utils\Number;
...
class Fire
{
	 ...
	 */
	private function getProbabilityToShotAgainForAttackerShipOfType(ShipType $shipA): int
	{
		$p = 0;

		foreach ($this->defenderFleet->getShips() as $shipD) {
			$RF = $shipA->getRfTo($shipD);
			$probabilityToShotAgain = 1 - GeometricDistribution::getProbabilityFromMean($RF);
			$probabilityToHitThisType = $shipD->getCount() / $this->defenderFleet->getTotalCount();
	...
	{
		return new self($this->attackerShipType, $this->defenderFleet);
	}
}
