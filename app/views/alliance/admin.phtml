...
<div class="container-fluid">
			...
			<tr>
				...
				<th>
					<select style="width:98%;" name="request_notallow" title="">
						<option value="1"<?=$parse['request_notallow_0'] ?>><?=_getText('No_allow_request') ?></option>
						<option value="0"<?=$parse['request_notallow_1'] ?>><?=_getText('Allow_request') ?></option>
					</select>
				</th>
			</tr>
			<tr>
	...
	<div class="separator"></div>
	<div class="row">
		<? if (isset($parse['Disolve_alliance'])): ?>
			<div class="col-xs-6">
				<?=$parse['Disolve_alliance'] ?>
			</div>
		<? endif; ?>
		<? if (isset($parse['Transfer_alliance'])): ?>
			<div class="col-xs-6">
