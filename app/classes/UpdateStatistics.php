...
class UpdateStatistics extends Injectable
{
	...
	public function inactiveUsers ()
	{
		...
		while ($user = $list->fetch())
		{
			...
			if (Helpers::is_email($user['email']))
			{
				...
				<br><br>Во избежании удаления аккаунта вам нужно будет зайти в игру и через <a href=\"http://uni" . $this->config->game->universe . ".xnova.su/?set=options\">настройки профиля</a> отменить процедуру удаления.<br><br>С уважением, команда <a href=\"http://uni" . $this->config->game->universe . ".xnova.su\">XNOVA.SU</a>";
				$mail->Send();
			}
	...
	{
		$this->db->query("DELETE FROM game_messages WHERE `message_time` <= '" . (time() - 432000) . "';");
