...
//!
//! This Rust library is called from Xnova NativeEngine.php via FFI (Foreign Function Interface)
...
///
/// Invalid input returns an allocated JSON error rather than aborting the PHP worker.
///
/// # Safety
/// `input_json` must be null or point to a valid NUL-terminated string for this call.
#[no_mangle]
pub unsafe extern "C" fn fight_battle_rounds(input_json: *const c_char) -> *mut c_char {
    let result = std::panic::catch_unwind(|| -> Result<String, String> {
        if input_json.is_null() {
            return Err("null battle input".into());
        }
        let input_str = unsafe { CStr::from_ptr(input_json) }
            .to_str().map_err(|error| error.to_string())?;
        let battle_input: BattleInput = serde_json::from_str(input_str)
            .map_err(|error| error.to_string())?;
        validate_input(&battle_input)?;
        serde_json::to_string(&process_battle_rounds(battle_input))
            .map_err(|error| error.to_string())
    }).unwrap_or_else(|_| Err("battle engine panicked".into()));

    let json = result.unwrap_or_else(|error| serde_json::json!({ "error": error }).to_string());
    CString::new(json).expect("serialized JSON cannot contain NUL bytes").into_raw()
}

// Keep the compact hot-loop indices and counters within their representable ranges.
fn validate_input(input: &BattleInput) -> Result<(), String> {
    for fleets in [&input.attacker_fleets, &input.defender_fleets] {
        let mut fleet_ids = HashSet::new();
        let mut info_count = 0usize;
        let mut unit_count = 0u32;
        for fleet in fleets {
            if !fleet_ids.insert(fleet.fleet_mission_id) {
                return Err("duplicate fleet id on the same side".into());
            }
            info_count += fleet.units.len();
            if info_count > u16::MAX as usize + 1 {
                return Err("too many fleet/unit types on one side".into());
            }
            for (id, unit) in &fleet.units {
                if *id != unit.unit_id || !unit.attack_power.is_finite()
                    || !unit.shield_points.is_finite() || !unit.hull_plating.is_finite()
                    || unit.attack_power < 0.0 || unit.shield_points < 0.0
                    || unit.hull_plating <= 0.0 || unit.rapidfire.values().any(|amount| *amount == 0)
                {
                    return Err("invalid combat unit values".into());
                }
                unit_count = unit_count.checked_add(unit.amount)
                    .ok_or("too many units on one side")?;
            }
        }
    }
    Ok(())
...
///
/// # Safety
/// `ptr` must be null or an unfreed pointer returned by `fight_battle_rounds`.
#[no_mangle]
pub unsafe extern "C" fn free_battle_result(ptr: *mut c_char) {
    if !ptr.is_null() {
        unsafe {
...
fn process_combat(
    ...
    for attacker in attackers.iter() {
        ...
        loop {
            ...
            let target_idx = rng.random_range(0..defenders.len());
...
fn fleet_results(infos: &[CombatUnitInfo], alive: &[u32]) -> HashMap<u64, FleetResult> {
    let mut results: HashMap<u64, FleetResult> = HashMap::new();
    for (idx, info) in infos.iter().enumerate() {
