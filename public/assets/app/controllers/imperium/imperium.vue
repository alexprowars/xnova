<template>
	<div v-if="page" class="page-imperium table-responsive">
		<table class="table">
			...
			<tr v-for="(i, res) in $root.getLang('RESOURCES')" v-if="res !== 'energy'">
				<th>{{ $root.getLang('RESOURCES', res) }}</th>
				<th v-for="planet in page['planets']">
			...
			<tr>
				<th>{{ $root.getLang('RESOURCES', 'energy') }}</th>
				<th v-for="planet in page['planets']">
			...
			<tr v-for="(item, id) in page['tech']">
				<th :colspan="rows - 1">{{ $root.getLang('TECH', id) }}</th>
				<th>
	...
	export default {
		...
		computed: {
			...
			total () {
				...
				let elements = Object.keys(this.$root.getLang('TECH'));
