<template>
	<tr class="planetRow">
			...
			<a v-if="item && !item.p_delete" class="tooltip_sticky">
				<div class="tooltip-content">
					<table width="240">
						...
						<tr>
							<th width="80">
								<img v-bind:src="$root.getUrl('assets/images/planeten/small/s_'+item.p_image+'.jpg')" height="75" width="75">
							</th>
							<th align="left">
				...
				<img v-bind:src="$root.getUrl('assets/images/planeten/small/s_'+item.p_image+'.jpg')" height=30 width="30">
			</a>
		</th>
		<th width="130">
										...
										<th colspan="2" align="center">
											<div v-if="item.u_id !== $root.user.id">
												...
												<div v-if="$parent.page.user.destroy > 0">
													<a v-bind:href="$root.getUrl('fleet/g'+$parent.page.galaxy+'/s'+$parent.page.system+'/p'+item.planet+'/t3/m9/')">Уничтожить</a>
												</div>
											</div>
											<div v-else>
												<a v-bind:href="$root.getUrl('fleet/g'+$parent.page.galaxy+'/s'+$parent.page.system+'/p'+item.planet+'/t3/m4/')">Оставить</a>
											</div>
											<a v-bind:href="$root.getUrl('fleet/g'+$parent.page.galaxy+'/s'+$parent.page.system+'/p'+item.planet+'/t3/m3/')">Транспорт</a>
										</th>
									</tr>
								</table>
							</th>
		...
		<th v-bind:class="[debris_class]" width="30">
						...
						<tr>
							...
							<th>
								<table class="table">
									...
									<tr>
										<th colspan="2" align="left">
											<a v-bind:href="$root.getUrl('fleet/g'+$parent.page.galaxy+'/s'+$parent.page.system+'/p'+item.planet+'/t2/m8/')">Отправить флот</a>
										</th>
									</tr>
								</table>
							</th>
				...
				<a style="cursor: pointer;" class="tooltip_sticky">
					<div class="tooltip-content">
						<table width="280">
							...
							<tr>
								<th valign="top">
									<a v-bind:href="$root.getUrl('stat/players/range/'+stat_page+'/pid/'+item.u_id+'/')">Статистика</a>
								</th>
							</tr>
						</table>
					</div>

					<span v-bind:class="[user_status_class]">{{ item.u_name }}</span>
				</a>

				<span v-if="user_status" v-bind:class="[user_status_class]">
					<font color="white">(</font><span v-if="user_status === 'UG' || user_status === 'G'"><a v-bind:href="$root.getUrl('banned/')" v-bind:class="[user_status_class]">{{ user_status }}</a></span><span v-else="">{{ user_status }}</span><font color="white">)</font>
				</span>
		...
		<th width="16">
			<a v-if="item && !item.delete && item.u_race" v-bind:href="$root.getUrl('info/70'+item.u_race+'/')">
				<img v-bind:src="$root.getUrl('assets/images/skin/race'+item.u_race+'.gif')" width="16" height="16" v-bind:alt='races[item.u_race]' v-bind:title='races[item.u_race]'>
			</a>
		</th>
		<th width="80">
			<a v-if="item && !item.delete && item.a_id" style="cursor: pointer;" class="tooltip_sticky">
				<div class="tooltip-content">
					<table width="240">
						...
						<tr>
							<th>
								<a v-bind:href="$root.getUrl('alliance/info/'+item.a_id+'/')">Информация</a>
							</th>
						</tr>
						<tr>
							<th>
						...
						<tr v-if="item.a_web.length">
							<th>
								<a v-bind:href="$root.getUrl(item.a_web)" target="_blank">Сайт альянса</a>
							</th>
						</tr>
					</table>
				</div>
		...
		<th style="white-space: nowrap;" width="125">
			<div v-if="item && item.u_id !== $root.user.id && !item.p_delete">
				...
				<a v-bind:href="$root.getUrl('buddy/new/'+item.u_id+'/')" title="Добавить в друзья">
				...
				<a v-if="$parent.page.user.spy_sonde && !item.u_vacation" title="Шпионаж" class="tooltip_sticky">
					<div class="tooltip-content">
						<center>
							<input type="text" v-bind:value="$parent.page.user.spy">
							<br>
				...
				<a v-bind:href="$root.getUrl('players/'+item.u_id+'/')" title="Информация об игроке">
					<span class="sprite skin_s"></span>
				</a>
				<a v-bind:href="$root.getUrl('fleet/shortcut/add/new/g/'+$parent.page.galaxy+'/s/'+$parent.page.system+'/p/'+item.planet+'/t/'+item.p_type+'/')" title="Добавить в закладки">
					<span class="sprite skin_z"></span>
				</a>
			</div>

			<a v-if="!item && $parent.page.user.colonizer" v-bind:href="$root.getUrl('fleet/g'+$parent.page.galaxy+'/s'+$parent.page.system+'/p'+item.planet+'/t0/m7/')" title="Колонизация">
				<span class="sprite skin_e"></span>
			</a>
		</th>
	</tr>
...
<script>
	export default {
		...
		computed: {
			...
			{
				var CurrentPoints 	= this.$parent.page.user.s_points;
				var RowUserPoints 	= this.item.stat_points;
				...
				if (this.item.u_ban > time && this.item.u_vacation > 0)
					return "UG";
				else if (this.item.u_ban > time)
					return "G";
			...
			debris_class: function()
			{
				...
				var debris = parseInt(this.item.p_metal) + parseInt(this.item.p_crystal);
