<template>
	...
	<div class="page-friends">
		...
		<UiPanel clip>
			<div class="friend-recipient">
				<span>{{ $t('pages.friends.new.player') }}</span>
				<Link :href="'/players/' + page.id">{{ page.username }}</Link>
			</div>
			<form class="friends-form ui-form-body" @submit.prevent="send">
				<div class="friends-editor-heading">
					<label for="friend-message">{{ $t('pages.friends.new.message') }}</label>
					<span>{{ form.message.length }} / 250</span>
				</div>
