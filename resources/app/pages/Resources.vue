	...
	<div class="page-resources">
							...
							<thead>
								<tr>
									...
									<th v-for="(res, index) in [...page.resources, 'energy']" :key="res" scope="col">
										<ModalLink navigate :href="'/info/' + (index + 1)" :title="$t('tech.' + (index + 1))" class="resources-label" :class="res"><component :is="resourceIcons[res]" aria-hidden="true" focusable="false"/>{{ $t('resources.' + res) }}</ModalLink>
									</th>
							...
							<tfoot>
								<tr class="resources-capacity">
									...
									<td v-for="res in page.resources" :key="res">
										<span :class="planet.resources[res].capacity > planet.resources[res].value ? 'positive' : 'negative'">{{ $formatNumber(planet.resources[res].capacity / 1000) }} k</span>
									</td>
									<td><span class="positive">{{ $formatNumber(planet.resources.energy.capacity) }}</span></td>
									<td></td>
								</tr>
								<tr class="resources-total">
									<td colspan="3">{{ $t('pages.resources.total') }}</td>
									<td v-for="res in page.resources" :key="res"><Colored :value="planet.resources[res].production"/></td>
									<td><Colored :value="planet.resources.energy.value"/></td>
									<td></td>
								</tr>
							</tfoot>
						</table>
					</div>
					<div v-if="!isVacation" class="resources-form-actions"><button type="submit" class="button" name="action" value="Y">{{ $t('pages.resources.calculate') }}</button></div>
				</form>
			</div>
		</div>
		...
		<div class="resources-status">
			<div class="resources-efficiency">
				<div class="resources-status-heading"><span>{{ $t('pages.resources.production_level') }}</span><strong>{{ page.production_level }}%</strong></div>
			...
			<div class="resources-technology">
				<Link href="/info/113" class="resources-label energy"><EnergyIcon aria-hidden="true" focusable="false"/>{{ $t('tech.113') }}</Link>
		...
		<div v-if="!isVacation" class="block resources-forecast">
			...
			<div class="content table-responsive">
				<table class="table resources-table">
					<thead><tr><th scope="col"></th><th v-for="period in periods" :key="period.label" scope="col">{{ $t('pages.resources.' + period.label) }}</th></tr></thead>
					<tbody>
						<tr v-for="res in page.resources" :key="res">
							<th scope="row"><span class="resources-label" :class="res"><component :is="resourceIcons[res]" aria-hidden="true" focusable="false"/>{{ $t('resources.' + res) }}</span></th>
							<td v-for="period in periods" :key="period.label"><Colored :value="planet.resources[res].production * period.hours"/></td>
						</tr>
					</tbody>
				</table>
			</div>
		...
		<div class="block resources-storage">
			<div class="title">{{ $t('pages.resources.storage_status') }}</div>
			<div class="content resources-storage-grid"><StorageRow v-for="res in page.resources" :key="res" :resource="res"/></div>
		</div>

		<div v-if="!isVacation" class="block resources-management">
			<div class="title">{{ $t('pages.resources.production_management') }}</div>
			<div class="content resources-management-actions">
				<button type="button" @click="shutdown('Y')" class="button"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" aria-hidden="true"><path d="M12 3v9M7 5a8 8 0 1 0 10 0"/></svg>{{ $t('pages.resources.production_on') }}</button>
				<button type="button" @click="shutdown('N')" class="button resources-shutdown"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true"><rect x="5" y="5" width="14" height="14" rx="2"/></svg>{{ $t('pages.resources.production_off') }}</button>
			</div>
		</div>
