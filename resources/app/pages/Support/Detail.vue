<template>
	...
	<div class="page-support page-support-detail">
		...
		<UiHeading class="support-heading">
			<div class="support-detail-heading">
				<span class="support-ticket-number">{{ $t('pages.support.detail.ticket', { id: page.item.id }) }}</span>
				<h1>{{ page.item.subject }}</h1>
			</div>
			<span class="support-status" :class="'status-' + page.item.status">{{ $t('pages.support.status.' + page.item.status) }}</span>
		</UiHeading>
		<div class="support-thread">
			<article class="support-message support-original">
				<div class="support-message-meta">
					<strong>{{ $t('pages.support.detail.original') }}</strong>
					<time v-if="page.item.created_at" :datetime="page.item.created_at">
			...
			<article
				...
				:class="{ 'is-own': message.user_id === state.user.id }"
			>
				<div class="support-message-meta">
					<a v-if="message.user_id" :href="'/players/' + message.user_id" target="_blank">{{ message.user }}</a>
		...
		<UiPanel clip :title="$t('pages.support.detail.answer_title')" v-if="page.item.status !== 0">
			<form class="ui-form-body" method="post" @submit.prevent="answer">
				<div class="support-editor-heading">
					<span>{{ $t('pages.support.form.message') }}</span>
					<span :class="{ 'is-invalid': form.message.length > 255 }">{{ form.message.length }} / 255</span>
				</div>
				<TextEditor name="message" v-model="form.message" :class="{error: v$.message.$error || form.errors.message}"/>
				<div v-if="v$.message.$error" class="ui-errors" role="alert">
					{{ $t(form.message.length > 255 ? 'pages.support.form.message_limit' : 'pages.support.form.message_required') }}
				</div>
