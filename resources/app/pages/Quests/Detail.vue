	...
	<div class="page-quest-detail">
		...
		<header class="quest-detail-heading">
			...
			<div class="quest-intro">
				<span class="quest-number">{{ $t('pages.quests.number', { id: page.id }) }}</span>
				<h1>{{ page.title }}</h1>
				<div class="quest-description" v-html="page.description"></div>
			</div>
		</header>
		<div class="quest-detail-grid">
			<section class="quest-panel">
				<header class="quest-panel-heading">
					<h2>{{ $t('pages.quests.tasks') }}</h2>
					<span>{{ completedTasks }} / {{ page.task.length }}</span>
				</header>
				<div
					v-if="page.task.length"
					class="quest-task-progress"
					role="progressbar"
					:aria-label="$t('pages.quests.task_progress')"
					:aria-valuenow="completedTasks"
					:aria-valuemax="page.task.length"
					aria-valuemin="0"
				>
					<span :style="{ width: completedTasks / page.task.length * 100 + '%' }"></span>
				</div>
				<ul class="quest-task-list">
					<li v-for="(task, index) in page.task" :key="index" :class="{ 'is-done': task[1] }">
						<span
							class="quest-task-status"
							:title="$t(task[1] ? 'pages.quests.task_done' : 'pages.quests.task_pending')"
							role="img"
			...
			<aside class="quest-panel quest-reward">
				...
				<div v-if="Object.keys(form.errors).length" class="quest-errors" role="alert">
					<div v-for="(error, key) in form.errors" :key="key">{{ error }}</div>
				</div>
				<div v-if="!page.errors" class="quest-finish">
					<button type="button" class="button" :disabled="form.processing" @click.prevent="finish">
						{{ $t('pages.quests.finish') }}
					</button>
				</div>
			</aside>
		</div>
