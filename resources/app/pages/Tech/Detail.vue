		...
		<header class="tech-detail-heading">
			...
			<div class="tech-detail-identity">
				...
				<div class="tech-detail-meta">
					<span>{{ $t('pages.techtree.current_level') }} <b>{{ page.level }}</b></span>
					<span class="tech-availability" :class="page.available ? 'is-ready' : 'is-missing'">
						{{ $t(page.available ? 'pages.techtree.fulfilled' : 'pages.techtree.missing') }}
					</span>
				</div>
			</div>
			<ModalLink navigate :href="'/info/' + page.id" class="button" aria-haspopup="dialog">
				{{ $t('pages.techtree.description') }}
			</ModalLink>
		</header>
		<section class="tech-dependencies">
			<header class="tech-dependencies-heading">
				<h2>{{ $t('pages.techtree.requirement_tree') }}</h2>
				<span>{{ $t('pages.techtree.level_hint') }}</span>
			</header>
			<ul v-if="requirements.length" class="tech-dependency-list">
				<DependencyNode
					v-for="requirement in requirements"
					:key="page.id + ':' + requirement.id"
