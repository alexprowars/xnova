	...
	<div class="game-page page-auth">
		<UiPanel class="game-panel">
			<header class="auth-heading">
				...
				<h1>{{ $t('pages.auth.recovery') }}</h1>
				<p>{{ $t('pages.auth.recovery_hint') }}</p>
			</header>
			<form class="game-form" @submit.prevent="send">
				<label>
					{{ $t('pages.registration.email') }}
					<input type="email" name="email" autocomplete="email" :class="{error: v$.email.$error}" v-model="form.email">
				</label>
				<div v-if="v$.email.$error" class="game-errors">{{ $t('pages.auth.valid_email') }}</div>
				<div v-for="(error, key) in form.errors" :key="key" class="game-errors">{{ error.message || error }}</div>
				<div class="game-actions">
					<UiButton type="submit" :disabled="form.processing">{{ $t('pages.auth.send_link') }}</UiButton>
				</div>
			</form>
		</UiPanel>
	</div>
