<template>
	...
	<div class="page-alliance ">
		...
		<header class="alliance-heading">
			<h1>{{ $t('pages.alliance.create.title') }}</h1>
		</header>
		<section class="alliance-panel">
			<form class="alliance-form" @submit.prevent="create">
				<div class="alliance-fields">
					<label>
						{{ $t('pages.alliance.create.tag_label') }}
						<input type="text" name="tag" :class="{error: v$.tag.$error}" maxlength="8" v-model="form.tag">
						<span v-if="v$.tag.$error" class="alliance-errors">{{ $t('pages.alliance.ui.required') }}</span>
					</label>
					<label>
						{{ $t('pages.alliance.ui.name_label') }}
						<input type="text" name="name" :class="{error: v$.name.$error}" maxlength="30" v-model="form.name">
						<span v-if="v$.name.$error" class="alliance-errors">{{ $t('pages.alliance.ui.required') }}</span>
					</label>
				</div>
