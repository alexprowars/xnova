<template>
	...
	<div class="page-stat">
		<div class="block stats-toolbar">
			<div class="title stats-heading">
				<span>{{ $t('pages.stats.title') }}</span>
				<span class="stats-updated">
					{{ $t('pages.stats.updated_at', { time: $formatDate(page.update, 'DD MMM YYYY HH:mm:ss') }) }}
				</span>
			</div>
			<div class="stats-filters">
				<label>
					<span>{{ $t('pages.stats.table_header_stats') }}</span>
				...
				<label>
					<span>{{ $t('pages.stats.rating_by') }}</span>
				...
				<label v-if="form.list !== 'races'">
					<span>{{ $t('pages.stats.players_table_rank') }}</span>
					<select v-model="form.page">
						<option v-for="i in form.pages" :key="i" :value="i">{{ (i - 1) * 100 + 1 }} – {{ i * 100 }}</option>
					</select>
				</label>
			</div>
		</div>
