<template>
	...
	<div class="page-merchant block">
		...
		<div class="content is-padded">
			<form method="post" @submit.prevent="exchange">
				<div class="merchant-intro">
					<div class="merchant-symbol" aria-hidden="true">
						<svg
							viewBox="0 0 24 24"
							fill="none"
							stroke="currentColor"
						...
						>
							<path d="M4 8h15l-4-4M20 16H5l4 4M19 8l-4 4M5 16l4-4"/>
						</svg>
					</div>
					<div>
						...
						<div class="merchant-fee">
							...
							{{ $t('pages.merchant.description_line_2') }}
						</div>
					</div>
				</div>
				<fieldset class="merchant-selection">
					...
					<div class="merchant-options">
						<label
							v-for="res in ['metal', 'crystal', 'deuterium']"
							:key="res"
							class="merchant-option"
							:class="[res, { 'is-selected': type === res }]"
						>
				...
				<div v-if="type !== ''" class="merchant-exchange">
					...
					<div class="merchant-table">
						<div class="merchant-table-heading" aria-hidden="true">
							<span></span>
							<span>{{ $t('pages.merchant.rate') }}</span>
							<span>{{ $t('pages.merchant.amount') }}</span>
						</div>
						<div
							v-for="res in ['metal', 'crystal', 'deuterium']"
							:key="res"
							class="merchant-row"
							:class="{ 'is-source': type === res }"
						>
							<label :for="'merchant-' + res" class="merchant-resource" :class="res">
								<ResourceIcon :code="res" aria-hidden="true" focusable="false"/>
								<span>{{ $t('resources.' + res) }}<small>{{ $t(type === res ? 'pages.merchant.give' : 'pages.merchant.receive') }}</small></span>
							</label>
							...
							<div class="merchant-quantity">
								<Number
									...
									:id="'merchant-' + res"
									min="0"
									v-model="resources[res]"
									:placeholder="$t('pages.merchant.quantity')"
									@input="calculate"
								/>
