	...
	<footer>
		<div class="hidden sm:block">
			<div class="w-full px-2 flex justify-between">
				<div>
					<Link href="https://github.com/alexprowars/xnova" target="_blank" title="Последние изменения">{{ state.version }}</Link>
				...
				<div>
					...
					<a @click.prevent title="Игроков в сети" style="color: var(--ui-success-color)">{{ state.stats['online'] }}</a>/<a @click.prevent title="Всего игроков" style="color: var(--ui-warning-color)">{{ state.stats['users'] }}</a>
				</div>
			</div>
		</div>
		<div class="sm:hidden footer-mobile">
			...
			<div class="w-full px-2 flex justify-between">
				<div>
					<Link href="https://github.com/alexprowars/xnova" target="_blank" title="Последние изменения">{{ state.version }}</Link>
					<a target="_blank" href="https://xnova.su/">© 2008 - {{ (new Date).getFullYear() }} Xcms</a>
				</div>
				<div>
					<a @click.prevent title="Игроков в сети" style="color: var(--ui-success-color)">{{ state.stats['online'] }}</a>/<a @click.prevent title="Всего игроков" style="color: var(--ui-warning-color)">{{ state.stats['users'] }}</a>
				</div>
