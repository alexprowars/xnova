<template>
	<div class="overview-queue-row">
		...
		<div class="overview-queue-description">
			...
			<button v-if="item.planet_id !== planet.id" type="button" class="overview-queue-planet is-link" @click="changePlanet(item.planet_id)">{{ planetItem?.name }} <span aria-hidden="true">↗</span></button>
		...
		<div class="overview-queue-time">
			...
			<div class="overview-queue-date" :title="$t('pages.building.queue_completion')">{{ $formatDate(item.date, 'DD MMM HH:mm:ss') }}</div>
		</div>
	</div>
</template>
