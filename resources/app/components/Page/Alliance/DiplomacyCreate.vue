<template>
	<section class="alliance-panel">
		<h2>{{ $t('pages.alliance.ui.add_relation') }}</h2>
		<form class="alliance-form" @submit.prevent="save">
			<div class="alliance-fields">
				<label>
					{{ $t('pages.alliance.ui.alliance') }}
					<select v-model="form.alliance">
						<option :value="null" disabled>{{ $t('pages.alliance.ui.choose_alliance') }}</option>
						<option v-for="item in items" :key="item.id" :value="item.id">{{ item.name }} [{{ item.tag }}]</option>
					</select>
				</label>
				<label>
					{{ $t('pages.alliance.ui.relation') }}
					<select v-model="form.status">
						<option v-for="status in [1, 2, 3]" :key="status" :value="status">
							{{ $t('alliance.diplomacy_status.' + status) }}
						</option>
					</select>
				</label>
			</div>
			...
			<div class="alliance-actions">
				<button type="submit" class="button" :disabled="!form.alliance || form.processing">
					{{ $t('pages.alliance.ui.add') }}
				</button>
			</div>
		</form>
	</section>
